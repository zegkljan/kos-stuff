...
def compute_gravity_turn(m0, m1, g0, r0, Isp, Fmax, cd, A, H, rho, h_obj, v_obj,
    ...
    :param vel_eps: initial velocity (must be nonzero, e.g. a very small number) (km * s^-1)
    ...
    quad = u
    dae = cs.SXFunction("dae", cs.daeIn(x=x, p=cs.vertcat([u, T])),
                        cs.daeOut(ode=T * cs.vertcat(ode), quad=T * quad))
    ...
    for i in range(0, N):
        ...
        frac = float(i + 1) / N
        x0 = x0 + u_init + [x0_init[i] + frac * (xf_init[i] - x0_init[i]) for i in
                            range(0, nx)]
    ...
    nlp = cs.MXFunction("nlp", cs.nlpIn(x=V),
                        cs.nlpOut(f=m0 - X[-1][0], g=cs.vertcat(G)))
